...
import { Package, Sparkles, Flame, Percent, Box, Edit3, Trash2, ChevronsLeft, ChevronLeft, ChevronRight, ChevronsRight } from 'lucide-react';
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-20 text-center">
          ...
          <p className="text-xl text-gray-500 font-medium">Chưa có sản phẩm</p>
        </div>
      </div>
    );
  }
          ...
          return (
            <div key={p.id} className="group relative bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-2xl hover:border-indigo-300 transition-all duration-300">
              ...
              <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => onEdit(p)} className="p-2.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg hover:bg-indigo-100"><Edit3 className="w-4 h-4 text-indigo-600" /></button>
                <button onClick={() => onDelete(p.id)} className="p-2.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg hover:bg-red-100"><Trash2 className="w-4 h-4 text-red-600" /></button>
              </div>
            </div>
          );
        })}
...
// Component Pagination riêng biệt
  ...
  <div className="border-t border-gray-200 px-6 py-5">
    <div className="flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => onPageChange(1)} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 text-black"><ChevronsLeft className="w-5 h-5" /></button>
        <button onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 text-black"><ChevronLeft className="w-5 h-5" /></button>
        {Array.from({ length: Math.min(7, totalPages) }, (_, i) => {
          let page = currentPage <= 4 ? i + 1 : currentPage - 3 + i;
          if (page > totalPages) return null;
          return (
            <button key={page} onClick={() => onPageChange(page)} className={`w-10 h-10 rounded-lg font-medium ${currentPage === page ? "bg-indigo-600 text-white shadow-lg scale-110" : "hover:bg-gray-100 text-gray-700"}`}>
              {page}
            </button>
          );
        }).filter(Boolean)}
        <button onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 text-black"><ChevronRight className="w-5 h-5" /></button>
        <button onClick={() => onPageChange(totalPages)} disabled={currentPage === totalPages} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 text-black"><ChevronsRight className="w-5 h-5" /></button>
      </div>
    </div>
  </div>
);
