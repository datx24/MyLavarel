...
import {
  ...
  Search, Filter, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Percent
} from "lucide-react";
...
interface Product {
  ...
  description: string;
  price: number;
  original_price?: number;  // THÊM MỚI
  stock: number;
  image?: string;
  category_id: number;
  is_new?: boolean;
...
export default function ProductsPage() {
  ...
  const openForm = (product?: Product) => {
    if (product) {
      ...
      setForm({
        name: "", price: "", original_price: "", stock: "", category_id: "", image: null, is_new: false, is_hot: false,
      });
  ...
  const saveProduct = async () => {
    if (!form.name.trim()) return toast.error("Nhập tên sản phẩm!");
    if (!form.price.trim()) return toast.error("Nhập giá bán!");
    if (!form.stock.trim()) return toast.error("Nhập số lượng!");
    if (!form.category_id) return toast.error("Chọn danh mục!");
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) return toast.error("Chỉ chấp nhận file ảnh!");
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 p-6">
                  {currentProducts.map((p) => {
                    const discount = p.original_price ? calculateDiscount(p.original_price, p.price) : null;
                    return (
                      <div
                        key={p.id}
                      ...
                      >
                        ...
                        <div className="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openForm(p)} className="p-2.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg hover:bg-indigo-100 transition-colors">
                            <Edit3 className="w-4 h-4 text-indigo-600" />
                          </button>
                          <button onClick={() => deleteProduct(p.id)} className="p-2.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-lg hover:bg-red-100 transition-colors">
                  ...
                  <div className="border-t border-gray-200 px-6 py-5">
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      ...
                      <div className="flex items-center gap-2 text-black">
                        <button onClick={() => goToPage(1)} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                          <ChevronsLeft className="w-5 h-5" />
                        </button>
                        <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                          <ChevronLeft className="w-5 h-5" />
                        </button>
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                          <button key={page} onClick={() => goToPage(page)} className={`w-10 h-10 rounded-lg font-medium transition-all ${currentPage === page ? "bg-indigo-600 text-white shadow-lg scale-110" : "hover:bg-gray-100 text-gray-700"}`}>
                            {page}
                          </button>
                        ))}
                        <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                          <ChevronRight className="w-5 h-5" />
                        </button>
                        <button onClick={() => goToPage(totalPages)} disabled={currentPage === totalPages} className="p-2 rounded-lg hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    ...
                    <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Nhập tên..." className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-black font-medium" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: e.target.value })} placeholder="0" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-4 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none transition-all text-black font-medium" />
                    </div>
                    <div>
                      <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2">
                        <Percent className="w-4 h-4" /> Giá gốc (tùy chọn)
                      </label>
                      <input type="number" value={form.original_price} onChange={(e) => setForm({ ...form, original_price: e.target.value })} placeholder="Để trống nếu không giảm" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-4 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all text-black font-medium" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="number" value={form.stock} onChange={(e) => setForm({ ...form, stock: e.target.value })} placeholder="0" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-4 focus:ring-purple-500/20 focus:border-purple-500 outline-none transition-all text-black font-medium" />
                  </div>
                  ...
                  <div>
                    ...
                    <select value={form.category_id} onChange={(e) => setForm({ ...form, category_id: e.target.value })} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all text-black font-medium">
              ...
              <div className="sticky bottom-0 bg-white border-t border-gray-200 p-6 flex justify-end gap-4">
                <button onClick={closeForm} className="px-6 py-3 border border-gray-300 rounded-xl font-semibold hover:bg-gray-50 transition-colors">
                  Hủy
                </button>
                <button
                  onClick={saveProduct}
